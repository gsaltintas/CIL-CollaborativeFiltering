...
case $JNB_SOFTWARE_STACK in
        ...
        new)
        if [ "$JNB_NUM_GPU" -gt "0" ]; then
            JNB_MODULE_COMMAND="gcc/8.2.0 r/4.0.2 python_gpu/3.8.5 cuda/11.3.1 bash eth_proxy $JNB_JULIA"
            echo -e "Using new software stack (gcc/8.2.0 python_gpu/3.8.5 eth_proxy $JNB_JULIA)"
        else
